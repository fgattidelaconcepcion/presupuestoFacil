...
export default function ObraDetailPage() {
  ...
  const [editEmpForm, setEditEmpForm] = useState({ name: "", dailyRate: "" });
  ...
  async function toggleAttendance(employeeId: string, day: string) {
    if (!currentPayroll) return;
    const newVal = !(attendances[employeeId]?.[day] ?? false);
    setAttendances((prev) => ({
      ...prev,
  ...
  async function addEmployee() {
    ...
    const res = await fetch("/api/empleados", {
      ...
      body: JSON.stringify({
        name: empForm.name,
        dailyRate: parseFloat(empForm.dailyRate),
  ...
  async function saveEditEmployee() {
    ...
    await fetch(`/api/empleados/${editingEmp.id}`, {
      ...
      body: JSON.stringify({
        ...
        dailyRate: parseFloat(editEmpForm.dailyRate),
      }),
    });
    setEmpLoading(false);
    setEditingEmp(null);
  ...
  function calcWeekTotal() {
    ...
    return project.employees.reduce(
      (sum, emp) => sum + calcDaysWorked(emp.id) * emp.dailyRate,
      0,
    );
  }
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-2xl">
            ...
            <div className="space-y-3">
              <input
                ...
                onChange={(e) =>
                  setEditEmpForm((p) => ({ ...p, name: e.target.value }))
                }
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                placeholder="Nombre"
              />
              <div className="relative">
                ...
                <input
                  type="number"
                  value={editEmpForm.dailyRate}
                  onChange={(e) =>
                    setEditEmpForm((p) => ({ ...p, dailyRate: e.target.value }))
                  }
                  className="w-full pl-7 pr-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                  placeholder="Jornal diario"
                />
              </div>
              <div className="flex gap-2 pt-1">
                <button
                    ...
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        ...
                        {DAYS_OF_WEEK.map((d) => (
                          <th
                            key={d.key}
                            className="text-center px-2 py-2.5 text-xs font-semibold text-slate-500 min-w-[40px]"
                          >
                            {d.label}
                          </th>
                        ))}
                      ...
                      {project.employees.map((emp, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {DAYS_OF_WEEK.map((d) => {
                              const present =
                                attendances[emp.id]?.[d.key] ?? false;
                              return (
                                <td
                                  key={d.key}
                            ...
                            <td className="text-center px-3 py-3">
                              <div className="font-bold text-slate-800 text-sm">
                                {formatCurrency(days * emp.dailyRate)}
                              </div>
                              <div className="text-xs text-slate-400">
                                {days}d
                              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEmpForm((p) => ({ ...p, dailyRate: e.target.value }))
                    }
                    min="1"
                    step="0.01"
                    className="w-full pl-7 pr-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm"
                  ...
                  >
                    <div className="px-4 py-3 flex items-center justify-between border-b border-slate-50">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            generarPDF(project, payroll, attendances)
                          }
