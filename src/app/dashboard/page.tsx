...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch('/api/obras').then(r => r.json()).then(data => { setProjects(data); setLoading(false); });
  }, []);

  const active = projects.filter(p => p.status === 'active');
  const finished = projects.filter(p => p.status === 'finished');

  const getBarColor = (p: Project) => {
          ...
          <div className="flex items-start justify-between mb-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                {p.status === 'finished' && (
                  <span className="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">Finalizada</span>
                )}
              </div>
              {p.description && <p className="text-xs text-slate-400 mt-0.5">{p.description}</p>}
            </div>
            <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded-lg">{p.employeeCount} emp.</span>
          </div>
          <div className="mb-2">
            ...
            <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${getBarColor(p)}`} style={{ width: `${pct}%` }} />
            </div>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-500">Restante: <span className="font-bold text-slate-800">{formatCurrency(p.budgetRemaining)}</span></span>
            <span className="text-slate-400 text-xs">Total: {formatCurrency(p.budget)}</span>
          </div>
        </div>
      </Link>
    );
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">{active.length} activa{active.length !== 1 ? 's' : ''}</p>
        </div>
        <Link href="/dashboard/obra/nueva"
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="font-semibold text-slate-700 mb-1">TodavÃ­a no tenÃ©s obras</h3>
          <p className="text-sm text-slate-400 mb-5">CreÃ¡ tu primera obra para empezar</p>
          <Link href="/dashboard/obra/nueva"
          ...
          {finished.length > 0 && (
            <div className="mt-6">
              <button onClick={() => setShowFinished(!showFinished)}
                ...
                <svg className={`w-4 h-4 transition-transform ${showFinished ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {showFinished && (
                <div className="space-y-3 opacity-75">
                  {finished.map(p => <ProjectCard key={p.id} p={p} />)}
                </div>
              )}
            </div>
          )}
